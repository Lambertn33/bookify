...
import { AppTextInput, AppButton, AppTitle, AppText, AppView } from '@/components/ui'
...
const AuthScreen = () => {
  ...
  const validateNames = (names: string): string => {
    if (!names) {
  ...
  const handleEmailChange = (text: string) => {
    if (isLogin) {
      ...
      setLoginState({
        ...loginState,
        email: text,
        errors: { ...loginState.errors, email: emailError },
      });
    } else {
      const emailError = validateEmail(text);
      setRegisterState({
        ...registerState,
        email: text,
        errors: { ...registerState.errors, email: emailError },
      });
    }
  };
  ...
  const handlePasswordChange = (text: string) => {
    if (isLogin) {
      ...
      setLoginState({
        ...loginState,
        password: text,
        errors: { ...loginState.errors, password: passwordError },
      });
    } else {
      const passwordError = validatePassword(text);
      const confirmPasswordError = registerState.confirmPassword
        ? validateConfirmPassword(text, registerState.confirmPassword)
        : '';
      setRegisterState({
        ...registerState,
        password: text,
  ...
  const handleConfirmPasswordChange = (text: string) => {
    const confirmPasswordError = validateConfirmPassword(registerState.password, text);
    setRegisterState({
      ...registerState,
      confirmPassword: text,
      errors: { ...registerState.errors, confirmPassword: confirmPasswordError },
    });
  };
  ...
  const handleNamesChange = (text: string) => {
    ...
    setRegisterState({
      ...registerState,
      names: text,
      errors: { ...registerState.errors, names: namesError },
    });
  };
  ...
  const handleClearErrors = () => {
    ...
    setRegisterState({
      ...registerState,
      errors: { names: '', email: '', password: '', confirmPassword: '' },
    });
  };
  ...
  const handleRegister = () => {
    console.log('Register:', { names: registerState.names, email: registerState.email, password: registerState.password, confirmPassword: registerState.confirmPassword })
  };
